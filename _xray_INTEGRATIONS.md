...
| **Motor Node v2 (Engines + Gateway)** | 🟢 Activa (dev) | **Autorización** | Sin `x-fifer-api-key` → **401**; llave inválida → **403** | UI: candado vía `fetchFiferEngine` / `getFinanceSnapshotData`; operación: registrar key en Supabase `api_keys` con scope `finance.cashflow` | 2026-04-09 |
