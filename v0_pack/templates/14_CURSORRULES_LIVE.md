...
Queda **ESTRICTAMENTE PROHIBIDA** la creación manual de carpetas, archivos base o planos sueltos (`_xray_*.md`) para nuevas aplicaciones, motores o sub-motores. Todo nuevo módulo **DEBE** nacer exclusivamente a través del motor de scaffolding oficial (p. ej. `npm run fifer:create-app` o el generador de motores del repo). El Starter Kit garantiza la inyección inmediata del módulo en la estructura base y evita código huérfano o planos desconectados del ADN.

## 22. PRINCIPIO "NACIDO CONECTADO"

Ninguna entidad arquitectónica en FIFER existe en aislamiento. Al inicializar un nuevo módulo o refactorizar uno existente, el agente **DEBE** asegurar automáticamente:

1. Su registro en el `EngineRegistry` cuando se trate de lógica/motor.
2. La declaración de su ancla `` `FIFER://...` `` bajo `## UBICACIÓN LÓGICA` en sus planos X-Ray.
3. La ejecución inmediata de `npm run sync:gps` para inyectarlo en `docs/registry/LOCATION_MAP.json`.
4. Su exposición a la **Sala de Guerra** (War Room) de forma que la telemetría lo capture mediante **descubrimiento dinámico** (sin listas estáticas en frontend; coherente con **§18**).

## 23. DEPURACIÓN ESTRICTA Y CÓDIGO MUERTO

### 23.1 Erradicación de falsos positivos (rutas y bundle)

El sistema principal de UI vive en `src/app/[locale]/(dashboard)`. Cualquier ruta antigua de dashboard que resida **directamente** en `src/app/(dashboard)` (sin `[locale]`), así como rutas obsoletas de `/afiliados` e implementaciones antiguas de `/contratos` que **no** utilicen `@/i18n/navigation`, deben **eliminarse o migrarse** en cuanto se toquen esas áreas, para no engordar el bundle ni confundir al GPS. No mantener duplicados paralelos salvo transición documentada en X-Ray.

### 23.2 Ley de tolerancia cero al código muerto

Al modificar código: eliminar variables no utilizadas y dependencias huérfanas en el momento en que se detecten. No dejar código comentado como "respaldo" salvo que esté documentado bajo un proceso de **deprecación explícita** en los planos X-Ray correspondientes.

## 24. PROTOCOLO DE AUTO-HEALING VISUAL (AUDITORÍA OBLIGATORIA EN UI)

En cada componente UI que se toque o refactorice, el agente debe auditar y aplicar lo siguiente (además de **§18**, **§19** y **§20**):
