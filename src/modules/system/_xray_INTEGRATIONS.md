...
| **Motor Node API v2 (Gateway `x-fifer-api-key`)** | Engines bajo `/api/v2/engines/*` (ej. finanzas cashflow) | `SUPABASE_URL` / `NEXT_PUBLIC_SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` en el proceso `npm run api` | **401** sin header o **403** llave inválida → shell Next marca `isLocked` y muestra candado (`fetchFiferEngine`). Desarrollo: conviene `PORT=3999` para no chocar con Next en 3000. |
