...
const BodySchema = z.object({
  ...
  progressReport: z
    .string()
    .min(1, 'progressReport no puede estar vacío.'),
  ownerId: z.string().min(1).optional(),
});

/**
 * POST /api/v1/ai-orchestrator/update — Fase 7: informe Cursor → nueva versión GEMINI_DOC.
 */
export async function POST(request: NextRequest) {
  ...
  const { sessionId, progressReport, ownerId: ownerIdFromBody } = parsed.data;
  ...
  try {
    ...
    const result = await orchestratorEngine.updateGeminiDoc(
      sessionId,
      progressReport,
    );
    return NextResponse.json(result, { status: 200 });
  } catch (error) {
    console.error('[API] /v1/ai-orchestrator/update', error);
    const message =
    ...
    return NextResponse.json(
      {
        error:
          status === 503
            ...
            : 'Error al actualizar el blueprint.',
