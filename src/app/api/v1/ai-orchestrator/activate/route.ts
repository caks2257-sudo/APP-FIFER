...
/**
 * POST /api/v1/ai-orchestrator/activate — Fase 4: genera el activador para Cursor.
 */
export async function POST(request: NextRequest) {
  ...
  const { sessionId, ownerId: ownerIdFromBody } = parsed.data;
  ...
  try {
    ...
    const result = await orchestratorEngine.generateGeminiActivator(sessionId);
    return NextResponse.json(result, { status: 200 });
  } catch (error) {
    console.error('[API] /v1/ai-orchestrator/activate', error);
    const message =
    ...
    return NextResponse.json(
      {
        error:
          status === 503
            ...
            : 'Error al generar el activador Gemini.',
