...
/**
 * POST /api/v1/ai-orchestrator/deploy — Fase 9: disparo de despliegue (Vercel hook opcional).
 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await orchestratorEngine.triggerDeployment(sessionId);
    return NextResponse.json(result, { status: 200 });
  } catch (error) {
    console.error('[API] /v1/ai-orchestrator/deploy', error);
    const message =
    ...
    return NextResponse.json(
      {
        error:
          status === 503
            ...
            : 'Error al disparar el despliegue.',
