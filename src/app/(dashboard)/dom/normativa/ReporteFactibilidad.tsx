  ...
  if (error) {
    return (
      <div
        ...
        role="alert"
      >
        <p className="font-semibold text-red-200/95">Error en el análisis</p>
  ...
  if (!result) {
    return (
      <div className="rounded-xl border border-dashed border-[#1E293B] bg-[#0A0F1E]/80 p-8 text-center text-sm text-[#64748B]">
        Ejecute el análisis para ver el reporte de factibilidad.
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-baseline justify-between gap-3">
        <h2 className="text-lg font-semibold text-[#F9FAFB]">Reporte de factibilidad</h2>
        <span
        ...
        >
          {result.factible ? 'Factible' : 'No factible'}
        </span>
      </div>

      <p className="mt-4 text-sm text-[#94A3B8]">
        Superficie máxima edificable (referencia calculada / modelo)
      </p>
      <p className="mt-1 font-mono text-2xl font-semibold tabular-nums text-[#F9FAFB]">
        {Number.isFinite(result.superficieMaximaEdificable)
          ? `${result.superficieMaximaEdificable.toLocaleString('es-CL', { maximumFractionDigits: 2 })} m²`
          : '—'}
      </p>
