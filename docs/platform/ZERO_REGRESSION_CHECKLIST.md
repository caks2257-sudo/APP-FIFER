...
| **HTTP (Lovable temp-frontend API)** | `GET /api/v1/system/health` — Command Center health (Supabase `fifer_platform` + Make webhook); gated by `FEATURE_SYSTEM_MONITOR`; optional `SYSTEM_MONITOR_API_KEY` (Bearer / `X-System-Monitor-Key`). Server: `fifer-landing/temp-frontend/server/index.js`, route: `src/api/routes/system.routes.js`. Dev: Vite proxies `/api` → `SYSTEM_MONITOR_API_PORT` (default 8787). |
| **HTTP (Master — v2.7–v2.9)** | `POST /api/v1/master/automated-play` — `X-FIFER-INTERNAL-KEY` (ISC/bots). `POST /api/v1/master/orchestrate` — **JWT Supabase** (`Authorization: Bearer`) vía `auth_supabase.requireSupabaseJwt`; `user_id` inyectado en `metadata`. Rutas: `src/api/routes/public/master.routes.js` + `fifer-landing/temp-frontend` Express. Frontend: `api_client` adjunta JWT; login `src/pages/Login.tsx` + `AuthContext`. |
...
| **Safe Boot (v2.6–v2.7)** | `src/system/boot_sequence.js` — env integrity + `health_xray` + **Paso C** Final Bridge (`probeMasterBridgeReadiness` si `FEATURE_AUTO_PROCESS=true`: `healthz` con `MASTER_API_BASE_URL` o modo in-process; fallo → solo warning). `CRITICAL_BOOT_ERROR` solo en env/vascular. `fifer-landing/temp-frontend/server/index.js` runs boot before `listen`. Escapes: `FEATURE_DRY_RUN=true`, `SAFE_BOOT_SKIP=true`. Runner: `npm run boot:check`. |
...
| `FEATURE_SYSTEM_MONITOR` | `GET /api/v1/system/health` + `SystemStatus` UI (`fifer-landing/temp-frontend`); does not touch ingest |
